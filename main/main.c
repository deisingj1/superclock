	...
	//	{
		...
		if(time_set) {
			...
			tm1637_set_number_lead_dot(lcd, time_number, true, toggle++%2 ? 0xFF : 0x00);
   			...
   			gpio_set_level(AUDIO_ON, toggle%2);
...
/**
 * @brief Set up GPIO states
 *
 */
void init_gpio() {
   // Turn bottom right LED on
   gpio_set_direction(LED_BR, GPIO_MODE_OUTPUT);
   //gpio_set_level(LED_BR, 0);
   
   // Turn top left LED on
   gpio_set_direction(LED_TL, GPIO_MODE_OUTPUT);
   //gpio_set_level(LED_TL, 0);
   
   // Turn bottom left LED on
   gpio_set_direction(LED_BL, GPIO_MODE_OUTPUT);
   //gpio_set_level(LED_BL, 0);

   gpio_set_direction(AUDIO_OUT, GPIO_MODE_OUTPUT);
   gpio_set_level(AUDIO_OUT, 0);
	
   gpio_set_direction(AUDIO_ON, GPIO_MODE_OUTPUT);
   gpio_set_level(AUDIO_ON, 1);

   make_noise();

   gpio_set_level(AUDIO_ON, 0);
}
